...
def parse_input(text=None, no_input=False):
    ...
    if len(text_split) == 2:
        ...
        raise user_error(
            f"Invalid input (possibly multiple background prompts): {text}"
        )
    try:
...
def filter_boxes(gen_boxes, scale_boxes=True, ignore_background=True, max_scale=3):
    ...
    for gen_box in gen_boxes:
        if isinstance(gen_box, dict):
            if not gen_box["bounding_box"]:
                continue
            name, [bbox_x, bbox_y, bbox_w, bbox_h] = (
                gen_box["name"],
                gen_box["bounding_box"],
            )
        ...
        if ignore_background:
            if (
                (bbox_w >= size[1] and bbox_h >= size[0])
                or bbox_x > size[1]
                or bbox_y > size[0]
            ):
        ...
        if (
            bbox_x < 0
            or bbox_y < 0
            or bbox_x + bbox_w > size[1]
            or bbox_y + bbox_h > size[0]
        ):
            # Out of bounds boxes exist: we need to scale and shift all the boxes
            print(
                f"**Some boxes are out of bounds: {gen_box}, scaling all the boxes to fit**"
            )
            scale_boxes = True
    ...
    if box_dict_format:
        ...
        bbox_bottom_y_max = max(
            [gen_box[1][1] + gen_box[1][3] for gen_box in gen_boxes]
        )
    ...
    for gen_box in gen_boxes:
        ...
        if box_dict_format:
            gen_box = {"name": name, "bounding_box": bounding_box}
        else:
